...
(function () {
  ...
  function ajaxSendImage(params, onRespSuccess, onRespError) {
    ...
    $.ajax({
      // url相对路径，由后端打到页面上
      url: app.root + app.url.userImageApi + '?_hjt=' + (new Date()).getTime(),
      type: 'POST',
  ...
  // 将要暴露的接口，放在下面
  app.api = {
    ajaxSendImage: ajaxSendImage,
    // ajaxCheckImage: ajaxCheckImage
  }
}());