...
window.wxshare = (function() {
  var opts = {
    ...
    onSuccess: null,
    onCancel: null,
    getSignUrl: '//api.benbun.com/wechat/jssign/',
    jsApiList: [
      'onMenuShareTimeline',
  ...
  function config(optsArg) {
    ...
    wx.onMenuShareTimeline({
      ...
      success: function() {
        window._hmt && window._hmt.push(['_trackEvent', app.store.enterType + 'wxshare', 'Timeline', '分享到朋友圈成功']);
        if (opts.onSuccess) {
          opts.onSuccess();
        }
      },
      cancel: function() {
        window._hmt && window._hmt.push(['_trackEvent', app.store.enterType + 'wxshare', 'Timeline', '分享到朋友圈取消']);
        if (opts.onCancel) {
          opts.onCancel();
        }
      }
    ...
    wx.onMenuShareAppMessage({
      ...
      success: function() {
        window._hmt && window._hmt.push(['_trackEvent', app.store.enterType + 'wxshare', 'AppMessage', '分享给好友成功']);
        if (opts.onSuccess) {
          opts.onSuccess();
        }
      },
      cancel: function() {
        window._hmt && window._hmt.push(['_trackEvent', app.store.enterType + 'wxshare', 'AppMessage', '分享给好友取消']);
        if (opts.onCancel) {
          opts.onCancel();
        }
      }
