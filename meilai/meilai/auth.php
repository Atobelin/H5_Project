...
if (empty($code)) {
    $res = postCurl('http://api.hongyu.ren/meilai/code', array('url'=>$locl), 'json');
    $resarr = json_decode($res, true);
    header('location:'.$resarr['data']['url']);
} else {
    $res = postCurl('http://api.hongyu.ren/wechat/auth', array('code'=>$code, 'pro'=>'meilai'), 'json');
    $resarr = json_decode($res, true);
...
$zl_openid = $_GET['zl_openid'] ?? '';
$zl_nickname = $_GET['zl_nickname'] ?? '';
